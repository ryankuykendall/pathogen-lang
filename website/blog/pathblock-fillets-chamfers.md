...
A [chamfer](/pathogen/docs#path-blocks-chamfers) replaces a corner vertex with a straight line. The incoming and outgoing edges are trimmed by a distance, and a line segment connects the two trim points. The result is a beveled corner.
...
Pass two distances to trim different amounts on the incoming and outgoing edges — [`chamfer(d1, d2)`](/pathogen/docs#path-blocks-chamferd1-d2-pathblock-projectedpath):
...
[`chamferAtVertex(index, distance)`](/pathogen/docs#path-blocks-chamferatvertexindex-distance-pathblock-projectedpath) targets a single corner. The index comes from the PathBlock's `.vertices` array:
...
A [fillet](/pathogen/docs#path-blocks-fillets) replaces a corner with a circular arc tangent to both edges. The trim distance is calculated from the radius and the half-angle between the edges:
...
[`filletAtVertex(index, radius)`](/pathogen/docs#path-blocks-filletatvertexindex-radius-pathblock-projectedpath) rounds a single corner:
...
[`ellipticalFillet(rx, ry, rotation)`](/pathogen/docs#path-blocks-ellipticalfilletrx-ry-rotation-pathblock-projectedpath) adds an ellipse rotation in radians:
...
[`ellipticalFilletAtVertex`](/pathogen/docs#path-blocks-ellipticalfilletatvertexindex-rx-ry-pathblock-projectedpath) targets individual corners, with an optional rotation parameter.
...
Both chamfers and fillets handle edge cases gracefully. From the [documentation](/pathogen/docs#path-blocks-edge-cases):
...
Because chamfers and fillets return PathBlocks, you can chain them with any other PathBlock method — [`.draw()`](/pathogen/docs#path-blocks-drawing-a-path-block), [`.drawTo()`](/pathogen/docs#path-blocks-drawing-a-path-block), [`.project()`](/pathogen/docs#path-blocks-projecting-without-drawing), [parametric sampling](/pathogen/docs#path-blocks-parametric-sampling), or [boolean operations](/pathogen/docs#path-blocks-boolean-operations):
...
The final post in this series covers [boolean operations](/pathogen/blog/pathblock-boolean-operations) — combining two closed paths using union, difference, intersection, and xor. Since everything returns a PathBlock, you'll see how these operations compose with the fillets and chamfers covered here.
