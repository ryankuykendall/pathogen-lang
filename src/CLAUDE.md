...
4. **Visual verify** — `npx tsx src/cli.ts <file> --json` first (read `warnings` and each layer's `records`/`commands`), then `--output-svg-file=<f>.svg --png=<f>.png` and Read the PNG to confirm the output renders correctly, paths are smooth, and edge cases produce reasonable visual results. Method and traps: `project-docs/developer-experience/pathogen-debugging-playbook.md`.
