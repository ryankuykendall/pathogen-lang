 ...
 * @param {string} dString - SVG path d attribute value
 * @param {number} precision - Number of line segments per curve/arc (default 8)
 * @returns {Float32Array} Flat array of [x1, y1, x2, y2, ...] segment pairs
 */
export function flattenToSegments(dString, precision = 8) {
  const segments = [];
  ...
  let lastCmd = '';
  let lastCpX = 0, lastCpY = 0; // last control point (for S/T smooth reflection)
  let i = 0;

  function next() { return tokens[i++]; }
  function peek() { return tokens[i]; }
  function hasNumbers() { return i < tokens.length && typeof tokens[i] === 'number'; }
 ...
 */
function subdivideCubic(out, x0, y0, x1, y1, x2, y2, x3, y3, steps) {
 ...
 * Uses adaptive step count based on control polygon length.
 */
function subdivideQuadratic(out, x0, y0, x1, y1, x2, y2, steps) {
  // Adaptive: estimate curve length from control polygon, max ~8px per segment
 ...
 */
function subdivideArc(out, x0, y0, rxIn, ryIn, xRotDeg, largeArcFlag, sweepFlag, x, y, steps) {
