...
- **A query block is lowercase and relative, as the docs have always said it is.** The same case-blindness lived at roughly twenty curve-argument reads in `path-transforms.ts`, which treat arguments as deltas from `start` throughout. So a block off an absolutely-authored layer transformed into the wrong shape while its `d` still looked right — measured across the method surface, ten disagreed with the identical curve authored relatively: `subPath`, `offset`, `reverse`, `startAt`, `scale`, `mirror`, `boundingBox`, `dash`, `outline` and `fillet`. Rather than patch twenty reads with three different shapes, `wrapCommands` now normalizes once, where layer geometry becomes a standalone block. That is what [Path Queries](#path-queries-path-queries) already promised in three places — "Path blocks always report lowercase relative commands", and `absolute` is "always false on blocks and projections" — while the runtime answered `true`. A layer `Command` still reports the case you authored. Nothing moves for lowercase input. ISSUE-026.
...
- **BREAKING: `mapSlice(length)` returns full windows only.** `[1, 2, 3, 4].mapSlice(2)` is now `[[1, 2], [2, 3], [3, 4]]`; it used to end with a short `[4]`. An array of `n` elements gives `n - length + 1` windows, and a `length` longer than the array gives `[]` (was the short leftovers). The method's main job is pairing neighbours, and there the short tail only caused harm: `let [outer, inner] = pair` bound `inner` to `null` on the last pass, so every such loop needed a `pair.length` check to be correct. **To keep the old output, pass `{ partial: true }`** — `mapSlice(2, { partial: true })` is byte-identical to the previous behaviour. The options object rejects unknown keys by name (`unknown key 'strict' (supported: partial)`), and a bare `mapSlice(2, true)` is told what shape it wants. **`length` must now be a positive integer**: it used to be rounded without a word, so `mapSlice(2.6)` made windows of 3 and `mapSlice(0.4)` rounded to 0 and then complained about "at least 1". Both are now `mapSlice() length must be a positive integer, got 2.6 — wrap a computed length in round()`; the `round()` hint appears only when rounding is the fix (not for `0`, `-2` or `NaN`), and a whole number that arrives as a float (`4 / 2`) is accepted. Kotlin's `windowed` (`partialWindows = false`), Rust's `windows` and Ruby's `each_cons` make the same choice. No published sample or doc example used `mapSlice`; stored workspaces that iterate the short slices will see fewer of them. Docs: Syntax ".mapSlice(length, options?)".

### Fixed

#### Core

- **A drawing function no longer writes a missing number into the path.** `circle(50, 50, inner)` with `inner` null compiled successfully — exit 0, no warning — to `a null null …`, and inside a `@{ }` block to `a undefined undefined … NaN NaN`; the browser then rejected the whole path with "Expected number" while the compiler reported success. `circle(10)` (one argument of three) produced `M NaN undefined …`, `sqrt(-1)` as a radius produced `a NaN NaN`, `polarLine(0deg, null)` quietly drew a zero-length line, and `tangentArc(null, 45deg)` wrote `A null null`. The Syntax page already promised that null "as a path argument throws a descriptive error"; raw path commands honoured that and the functions did not. Every path function and every context-aware function now stops with a positioned error that names the function, the argument and — when the argument is a plain variable — the variable to chase: `Line 49, col 7: circle(): argument 3 (`inner`) is null — path functions need a number here`, or `circle() expects 3 arguments, got 1`. A *missing* argument to a context-aware function needed a different check: those are cases of one switch rather than function objects, so there is no arity to read, and the declared signatures do not mark optional parameters. A missing argument always comes out as NaN, so the guard inspects what the call produced — emitted path text, coordinates handed back to the program (`polarPoint`), and the heading itself, since `turn()` emits nothing and poisons the pen's direction instead — and reports how many arguments arrived: `polarLine() produced a non-numeric coordinate (NaN) — check its arguments (it received 1 argument)`. Code review found that hole; the first version checked only the arguments that were present. The rule lives in one module (`src/evaluator/path-emit-guard.ts`) and recognises the shape helpers by function identity, derived from the stdlib export, so a helper added later is covered on arrival and a user function that shadows a stdlib name is not misjudged. Swept against all 511 tracked `.pathogen` sources: none is newly rejected. Docs: Syntax "Null" → "Error Behavior".

#### Development

- **A path-blocks test had been passing on garbage.** `drawTo() › works with stdlib path functions` called `circle(10)` and asserted only the `M 50 50` prefix, so `M NaN undefined a undefined …` satisfied it. The new guard rejected the call and exposed it; the test now makes a valid call and asserts the exact output.
- **The guard's test matrix is derived, not listed**: every fixed-arity function in `pathFunctions` × every argument position, and every name in `contextAwareFunctions`, with a check that the matrix is not empty. The too-few-arguments matrix asserts the contract itself — the call either stops the compile or puts nothing non-numeric in the path — and calls each function as a *statement*: written as `let got = polarLine(…)` the segment is captured as a value and never emitted, and the matrix passed with the leak wide open.
- **Logged, not fixed: ISSUE-023** — a raw path argument still accepts NaN and Infinity (`M NaN 0`). Rejecting it is six call sites in one function, and was written and then reverted: five tests pin those outputs as the way they observe a math function's documented degenerate value, and making the argument fatal would turn "one broken path" into "no image" for generative programs. It is a policy decision (error, warning, or warning-and-drop), and it should be made once for raw arguments and drawing functions together; `project-docs/known-issues.md` has the options.
- **Logged, not fixed: ISSUE-022** — a method called directly on an array *literal* (`[1, 2].slice('a')`) reports its error with no line or column, because a method call takes its position from its receiver and an array literal carries none. It predates this work and affects every array method; `project-docs/known-issues.md` has the receiver matrix.
...
- **Blog series "Drawing Without Bookkeeping", parts 3–5: three domain projects.** Each draws its form in real units inside a scaled group layer, so layer queries answer in millimetres, and lets the twin do the domain's paperwork. Part 3, *A Linkage That Dimensions Itself* (`/blog/a-linkage-that-dimensions-itself`, `post54/`): a four-bar linkage as a labelled block drawn in both panels; pivots named by subscription, a `dimension(link, offset)` helper written once from the segment's ends, length, label and normal, the crank angle from the leaving bar's tangent and the transmission angle from the joint's turn, a five-position sweep annotated by the same subscriptions, and a worksheet that runs the Grashof test on the queried lengths. Part 4, *The Panel Prints Its Own Drill Schedule* (`/blog/the-panel-prints-its-own-drill-schedule`, `post55/`): a Eurorack panel with HP as the parameter; holes numbered and crossed by a subscription on `call(circle)`, the drill schedule printed from each hole's block, a legend from the `as segment` names, and the 10HP wiggle. Part 5, *The Fretboard Is a Formula* (`/blog/the-fretboard-is-a-formula`, `post56/`): 22 slots from one scale length by the twelfth root of two; frets numbered by ordinal, distances to 0.01 mm under each slot with per-text rotation, marker frets by an `:nth` list with dots in the space before, a 24.75-inch wiggle, and a fanned-fret finale whose leans come from each slot's tangent. Friction log entries 16–24; review dispositions in `project-docs/observable-reactive-paths/reviews/`.
...
- **`validate:samples` flags a label that crosses a panel divider.** A hairline spanning most of the canvas is a divider, and a label straying across it has left its column; the area test cannot see a 0.5-wide stroke, so check 3a warns on any overlap.
...
- **A text layer's transform reaches its `<text>` elements.** `rotate`, `translate-*`, `scale-*` and an explicit `transform` on a `TextLayer` were computed by the evaluator and then dropped by both emitters (the CLI string writer and the shared DOM builder the playground and VS Code use); path and group layers honoured them and the docs promised them for text layers. Both emitters now compose the layer transform with a per-text rotation, `transform="rotate(-90) rotate(30, x, y)"`.
...
- **Compiles can be cancelled.** The compile worker never stopped a compile once started: editing during a long one queued another full compile behind it, and stale results were discarded only after they finished and were cloned to the main thread. The editor now owns its own worker (`editorCompiler`, a `CompilerWorkerClient` instance in `playground/services/compiler-worker.ts`; the shared instance still backs the publish precheck and admin views unchanged). A newer compile terminates the running worker before posting (`[compiler-worker] cancelled superseded compile` in the console), a **Cancel** control beside the `Compiling…` chip — in the breadcrumb and the fullscreen chrome — stops a long compile by hand (chip reads "Cancelled", the last render stays on screen marked stale, no error panel), and a request that is already stale is refused before it is posted. Cancellation rejects with `CompileCancelledError` (`name: 'CompileCancelled'`), which the view treats as silence, never as an error; a crashed worker still surfaces as an error. No timer ever kills a compile. Tests: `tests/compiler-worker-cancel.test.ts`, `tests/compilation-status.test.ts`, `tests/app-breadcrumb-compile-clock.test.ts`, `tests/svg-preview-pane-compile-clock.test.ts`.
- **Conic (and mesh, freeform, topo) gradients no longer render blank above a 32768-unit viewBox.** `clampScale` floored the reduced texture scale at 0.25, so a 48000-wide viewBox asked WebGPU for a 12000-wide texture over its own 8192 cap; Dawn rejected it with uncaptured validation errors, nothing threw, `toDataURL` returned a transparent PNG and that blank was cached. The floor is gone (`playground/gpu/raster-size.ts`), every WebGPU render runs inside error scopes (`playground/gpu/gpu-error-scopes.ts`) with a size pre-check so a failure throws and the Canvas 2D fallback actually runs, a failed render is never cached, and the device now requests the adapter's texture limit (up to 16384 — a 48000-wide viewBox renders at 16384×6349 instead of 8192×3174). The texture cache keys on the post-clamp size and the renderer (`gpu` / `2d`). When a gradient falls back to Canvas 2D or cannot be rasterized at all, the Pathogen console says so (`[warn] Gradient 'wheel' (conic) …`) instead of only DevTools. `?gpu=off` (or `localStorage.pathogenForceCanvas2D = '1'`) forces the Canvas 2D path to preview what a browser without WebGPU shows; the CLI's `--render-gpu` honours the same switch as `PATHOGEN_GPU=off`.
- **PNG/PDF export no longer triggers Chrome's `willReadFrequently` warning.** Verification of a GPU-backed raster canvas now always goes through the CPU-backed downscale probe (one `drawImage` + one readback) instead of a 25-point sample grid of 1×1 `getImageData` calls on the accelerated canvas.

### Changed

#### Core

- **`normal(t).angle` is wrapped into the same (−π, π] range as `tangent(t).angle`.** It used to be the tangent minus π/2 with no re-wrap, so a normal facing the bottom-left quadrant came back below −π (down-left was −1.25π, exactly left −π) — and because nothing was ever reported above 0.5π, a `switch` arm such as `case 1.2pi..<1.8pi` on a normal could never match, which cost a user two rounds of debugging. Only bottom-left-facing normals change value (−1.25π → 0.75π; exactly left is now +π, matching what `tangent(t)` reports); every direction-only use (`cos`/`sin`/`heading`/`rotate` of the angle) is unaffected, and a range above π still never matches — the upper half of the circle is written with negative values. `docs/path-blocks.md` now states the range for both methods with a screen-space (y-down) orientation table and the `switch`-range trap; `docs/syntax.md` lists which angle producers are wrapped (`atan2`, `.angleTo()`, `tangent`, `normal`) and which accumulate (`PolarVector.angle`, `ctx.heading`). Helper `wrapToPi` in `src/evaluator/sampling.ts`; hover text regenerated. Tests: `tests/wrap-to-pi.test.ts`, `tests/path-blocks.test.ts` (down-left normal, exactly-left parity with the tangent, full-circle range sweep on arcs and lines in both windings, the switch case). Diagram: `project-docs/glyph-halo-diagnosis/normal-angle-and-winding-after-issue-020.pathogen` (ISSUE-020).
- **Conic gradients render the same on every surface.** `innerRadius`, `innerFill`, `spread` and `direction` were honoured only by the playground's WebGPU shader; the CLI / VS Code wedge renderer ignored `innerRadius` and `spread` entirely (and reversed `direction` by flipping stop offsets rather than reflecting the angle), and the Canvas 2D fallback ignored `direction`, `spread`, `innerRadius` and `innerFill`. New `src/conic-param.ts` is a line-for-line port of the shader's per-angle rules (angle wrap, `ccw` reflection, sweep division, `clamp` / `repeat` / `transparent`, inner-fill modes and the smoothstep blend). `src/conic-renderer.ts` now builds every wedge from it: the gap outside a partial sweep is painted for `clamp` (last stop) and `repeat` (tiled ramp), `innerRadius` with the default `'transparent'` fill cuts annular sectors, `'center'` / custom-color fills add a `<radialGradient>` disc following the shader's blend curve at five stops, `'transparent-blend'` masks the wedges the same way, and stop colors mix in gamma-encoded sRGB with straight alpha — the space the shader actually mixes in (its stops are canvas bytes uploaded as-is; the helper that feeds it was misnamed `cssColorToLinearRGBA` and is now `cssColorToRGBA`) and the default for CSS gradients (previously OKLCh when available, nearest-stop otherwise). `src/render/build-defs.ts` emits the extra defs, so the VS Code preview inherits the fix; the CLI's "ignored in CLI output" warning is gone because nothing is ignored. Adjacent wedges now overlap by 0.6 of a step (the last wedge painted keeps its true edge), which removes the faint seams SVG anti-aliasing drew between them. The playground's Canvas 2D fallback draws those same wedges through `Path2D` (the shared `renderConic` is exported on `window.PathogenLang`). Measured on the `project-docs/conic-parity/` fixture at 1,440 sampled pixels: WebGPU vs CLI wedges and WebGPU vs Canvas 2D both differ by a mean of 0.1 and at most 2 of 255 (scenario 5 of `scripts/debug-compile-cancel-and-conic.ts`). Tests: `tests/conic-param.test.ts`, `tests/conic-renderer.test.ts`, `tests/render/build-defs.test.ts`, `tests/cli.test.ts`, `tests/gpu-raster-size.test.ts`, `tests/gpu-error-scopes.test.ts`, `tests/gpu-texture-cache.test.ts`, `tests/gpu-gradient-notices.test.ts`.

#### Documentation

- `docs/gradients.md`: the conic "Rendering" sections describe the three renderers and their shared rules (including the mixing space), the raster resolution rule (viewBox × 2, reduced to the device's maximum texture edge — 16384 on most desktop GPUs, 8192 at minimum; 16384 for Canvas 2D), `?gpu=off`, and no longer promise a CLI warning for ignored properties. `docs/cli.md`: `PATHOGEN_GPU=off` for `--render-gpu`.
...
- **Cloudflare Pages builds pin npm through `packageManager`.** The v3 build image kept its baked-in npm 10.9.2 even after installing Node 24 from `.node-version`, and `npm clean-install` rejected a lockfile written by npm 11 (`Missing: @emnapi/runtime@1.11.3 from lock file`). `NPM_VERSION` is ignored by that image whether set in `wrangler.toml [vars]` or as a project variable; `"packageManager": "npm@11.6.2"` in `package.json` is honoured and the build now detects npm 11.6.2. `.puppeteerrc.cjs` skips Puppeteer's browser downloads when `CF_PAGES=1` (dependency install 27 s → 16 s).
