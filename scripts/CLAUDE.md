...
| `debug-workspace-switch-undo.ts` | Puppeteer repro + verification for the workspace-switch data-loss fixes: undo in workspace B must never restore workspace A's code (fresh-EditorState isolation), pending edits are flushed on in-app switches, the `?state=`/404 branches can't autosave foreign code into the previous workspace (stale debounce/keepalive timers), and leaving the workspace view then returning to the same workspace re-arms autosave (edits after returning must persist). Creates throwaway workspaces per scenario via the dev API. `--slow` extends observation windows past the 30s autosave MIN_INTERVAL. Run via `npx tsx scripts/debug-workspace-switch-undo.ts` (requires `npm run dev:stack` and a `PATHOGEN_API_BASE=http://localhost:8787 npm run build:playground`). |
