...
| `debug-compile-cancel-and-conic.ts` | Puppeteer verification for ISSUE-016 + ISSUE-017: a 48000×18600 viewBox renders its conic wheel through WebGPU with no Dawn texture/swapchain errors and a non-blank raster; `?gpu=off` renders through Canvas 2D and the Pathogen console carries one fallback notice per gradient; on a heavy program the breadcrumb's Cancel control stops the compile (chip → "Cancelled" → idle, no error panel) and an edit during a compile logs `[compiler-worker] cancelled superseded compile`; a PNG export never trips Chrome's `willReadFrequently` warning. Creates throwaway workspaces via the dev API (the heavy program is too long for `?state=`). `--only 1,3` selects scenarios, `--dump` prints the browser console. Run via `npx tsx scripts/debug-compile-cancel-and-conic.ts` (requires `npm run dev:stack` and a `PATHOGEN_API_BASE=http://localhost:8787 npm run build:playground`). |
