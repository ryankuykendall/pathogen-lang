...
| **Diagnostic notes (iter-3d-en, partial)** | The wedge artifact at Libre Bask UPPER EN tracking 0.5 was traced through `buildIntersectionLinks` and `traceContours` debug output across two iterations. **Iter-3d-en-1 (resolved)**: shared edge 1's `A.seg26[0.006..0.444]` ↔ `B.seg9[0.000..1.000]` asymmetry — fixed by the line-line endpoint snap (commit `4a20ca0`). **Iter-3d-en-2 (resolved)**: an unrelated cross-path vertex-vertex match at (56.72, 0) where A.seg26 t=0.0059 and B.seg8 t=1.0 referred to the same physical vertex on B but a non-vertex parametric point on A — fixed by a cross-path drop predicate added to `findAllIntersections` (drops non-boundary intersections where both `dist(point→segA endpoints) < 0.5` AND `dist(point→segB endpoints) < 0.5`). No test regressions; verified by inspecting that the dropped intersections are genuine vertex-vertex matches, not the unilateral near-vertex case that broke `radialWedge xor` in iter-3d-en-1's overzealous snap. **Iter-3d-en-3 (residual, NEXT PICKUP)**: run-count asymmetry persists at A=4 / B=3 even after both snaps. The wedge corresponds specifically to `runs[1]` on A: 4 cmds tracing E's bottom-right serif curves from (49.15,−6.95) to (57.04,0). The midpoint of that run sits inside N's left-stem interior (covered by N's filled region), so for a union the splits should be classified `'inside'` and dropped — leaving A with 3 runs to match B's 3 and producing a clean perimeter. They are instead classified `'outside'` and kept, which produces the standalone closed contour visible as the wedge. **The fix is not in intersection management — it's in `classifyByRingWalk` or its perpendicular-offset repair pass.** Likely candidates: (a) walk seed picked the wrong inside/outside on the ring containing run[1]'s splits, propagating wrong parity; (b) one isCrossingAtBoundary flip in the walk over those segments returned false when it should have returned true (or vice versa); (c) the perpendicular-offset repair's `unanimous` check is unintentionally bailing on those splits because their tangent samples land near another shared boundary. Pickup point: print `classes[i]` for each split index in run[1] and the splits adjacent to it on the ring, then trace which step set those values. Files: `boolean-ops.ts` `classifyByRingWalk` (~line 1820 walk loop, ~line 1880 repair pass). |
