...
| **Diagnostic notes (iter-3d-en, partial)** | The wedge artifact at Libre Bask UPPER EN tracking 0.5 was traced through `buildIntersectionLinks` debug output. Confirmed: A's outer ring produces 4 non-complete runs while B's produces 3, leaving one A-run with no Hungarian-matched B-partner. The contour tracer's implicit `z`-close on the resulting open chain emits a straight bridge — visible as the small triangular "tongue" hanging below the baseline at the E-N junction. Initial hypothesis (asymmetric shared-edge endpoint splits) was verified for shared edge 1 (`A.seg26[0.006..0.444]` ↔ `B.seg9[0.000..1.000]`) and resolved by the snap above, but the run-count asymmetry persists. Remaining candidate sources: (a) discrete intersections between A.seg26's neighbor segments (seg25, seg27) and B.seg8/seg10 producing non-vertex t-values on one side only; (b) an `inside`-classified non-boundary split that breaks the ring on A but not B. A broader vertex-snap in `splitPathAtIntersections` (snap any t within 0.5 px of segment endpoint) was attempted and reverted — it broke the `xor of radialWedge sharp vs rounded` test (chord 58.24 > 55), confirming that not every near-vertex t-value is safe to drop. The fix needs to be more discriminating: only snap intersections whose corresponding t on the other path is *also* near a vertex (a true vertex-vertex match), not unilateral near-vertex intersections. Pickup point for next iteration: `splitPathAtIntersections` lines ~1605–1645, but the snap needs cross-path coordination via `intersections` array, not just per-segment t-list operations. |
