...
| B2 | #12 root | Truthful startPoint = first inked point (audit revived; user chose ONE COHERENT RULE — flows into drawTo/mirror/scale; drawTo now anchors ink at target); firstInkedPointOf in segments.ts, both evaluators, endPoint copy-paste ride-along; annotated bugs 2-3 logged as #19 | LANDED (2026-08-26) |
...
| G | #8 | Query pseudo-selectors `:atomic` (né :each, renamed 2026-08-26) / `:first` / `:last` / `:nth(k)` (user chose full family; position trio kept deliberately as feature-set anchor after duplication audit) — parseSegmentQuery + queryLabeledRuns wrap findLabeledRuns; 3 dispatch sites; point/vertex guard | LANDED (2026-08-25) |
...
| F2 | #7b | Annotated label-name validation parity — shared labelNameError core, both PathCommand sites, 9 parity tests | LANDED (2026-08-26) |
