...
## Publishing
All five posts are committed on `main`, nothing pushed. `npm run build:blog` is warning-free; `validate:samples` is clean for post52–post56; the `public/` output was rebuilt with `PATHOGEN_API_BASE=http://localhost:8787` so Ryan's running `dev:stack` serves the series. The link checker last ran clean at parts 1–2 (1430 links); the run against the full series is recorded in the session recap. Publishing = `git push` (Pages auto-deploys).

## Deferred (language candidates surfaced by the series)
`circleCircle()` helper; `Command.heading`; `toFixed(n)`; per-subpath markers; per-call text anchor / column primitive; friendlier parser hint for `A.x`/`c.x` in path position; formatter residual (entry 20).
