...
Both forms take an optional third argument, the rotation, and an optional fourth, a style block for that one text — `text(40, 20, 0deg, #{ text-anchor: end; })` right-aligns a single label on a start-anchored layer. When there is no rotation, the style block can be the third argument: `text(40, 20, #{ text-anchor: end; })`. See [Per-Element Styles on Text and Tspan](#layers-per-element-styles-on-text-and-tspan).
