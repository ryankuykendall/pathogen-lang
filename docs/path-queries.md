...
- **`call` sees the outer statement only.** `call(circle)` finds `circle(...)` statements you wrote; it does not see a `circle()` inside a user function you called — that statement answers to `call(myFn)`. Boolean results (`union`, `cut`, ...) carry labels but not call identity.
